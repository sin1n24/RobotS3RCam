...
- **専用コントローラキット**: 指先に収まる[小型コントローラ(キット)](https://ssci.to/9521)を使えば、ジョイスティックとボタンによる直感的な操作と、L/Rモード（画面・操作の180度反転）にも対応可能です。※小型コントローラは向きやボタン配置など12通りから選択して組み立てることが出来ます。
